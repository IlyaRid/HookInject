...
int load_lib_by_pid(DWORD processId) 
{
    ...
    LPVOID argLoadLibrary = (LPVOID)VirtualAllocEx(openedProcess, NULL, strlen(dllName), MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE); //выделяем память под аргумент LoadLibrary, а именно - строку с адресом инъектируемой dll
